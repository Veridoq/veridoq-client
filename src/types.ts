...
export interface VerifyJobResponse {
  success: boolean;
  message: string;
  job: { id: string; status: string; documentName: string; createdAt: string };
  template?: { id: number; name: string; version: number; criteriaCount: number; isGlobal?: boolean; category?: string };
...
export interface V1TemplatesResponse {
  globalTemplates?: { count: number };
  sharedTemplates?: { count: number };
...
export type ChatStreamEvent =
  ...
  | { event: "done"; sessionId: string; documentIds: string[] }
